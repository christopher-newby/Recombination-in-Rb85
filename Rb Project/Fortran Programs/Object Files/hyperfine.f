...
c       Working variables
	...
	double precision dB,Bmax,Bfield
	integer nB
	common/Bgrid/ dB,Bfield(200),nB
...
c       Psvector(ichan=1,5; iF=1(F=4), iF=2(F=5) )
	...
	print*,' Ps-matrix:'
	do i=1,nchan
	   write(6,900)(Psmatrix(i,j),j=1,nchan)
	enddo
	
	print*,' Pt -matrix:'
	do i=1,nchan
	   write(6,900)(Ptmatrix(i,j),j=1,nchan)
	enddo
	
	
	return
 900	format(5(e18.12,2x))
	end
c       
C-----------------------------------------------------------------------
c
...
c---------------------------------------------------------------------------c
	...
	IMPLICIT REAL*8 (f,l,h,o,a,w,b,c)
	IMPLICIT INTEGER (i)
...
c       Here are the constant elements (h1-4)
	l11=0d0
	l13=0d0
	l33=0d0
